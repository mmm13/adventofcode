...
# Each present also requires a bow made out of ribbon as well; the feet of ribbon required
...
# requires 1+1+1+1 = 4 feet of ribbon to wrap the present
...
open(DATA, "<input.txt") or die "Couldn't open file input.txt, $!";
$ttl_sq_ft_ribbon=0;
while(<DATA>)
{
   #print "$cnt :  $_";
   ...
   $wh=$w+$w+$h+$h;
   $hl=$h+$h+$l+$l;
   $minPeri = min ($lw,$wh,$hl);
  #  print "length $l \n";
  #  print "width $w \n";
  #  print "height $h \n";
   print $minPeri,"\n";
   $pkg_sq_ft_ribbon= $minPeri + ($l * $w * $h);
   print "$pkg_sq_ft_ribbon  \n";

   $ttl_sq_ft_ribbon = $ttl_sq_ft_ribbon + $pkg_sq_ft_ribbon;

}#while
print "ttl: $ttl_sq_ft_ribbon \n";
close DATA;
